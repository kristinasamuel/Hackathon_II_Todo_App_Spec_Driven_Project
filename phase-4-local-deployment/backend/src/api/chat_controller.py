...
# Import ai_agent with error handling
try:
    ...
    async def process_user_message(user_id: str, message: str, conversation_history=None):
        return "AI functionality is not available due to missing dependencies."

    logger.warning("AI agent is not available. Chat functionality will return placeholder responses.")
    ...
    # Define a dummy function to prevent runtime errors
    async def process_user_message(user_id: str, message: str, conversation_history=None):
        return "AI functionality is not available due to unexpected error."
...
def process_chat_request(
    ...
    try:
        ...
        if not AI_AGENT_AVAILABLE:
            # AI agent is not available, return a helpful message
            ai_response = "AI functionality is not available due to missing dependencies. Core task operations are still available."

            # Create a fake conversation for the response
            ...
            return ChatResponse(
                ...
                action_taken="AI agent not available - returned informational response"
            )
        ...
        try:
            ...
            agent_instance = TodoChatAgent()  # This will try to initialize the model

            # Check if the model was successfully initialized
            if agent_instance.model is None:
                # Model wasn't initialized, but that's OK - we'll handle it gracefully
                pass
        except Exception as e:
            # Log the error but don't raise an exception - handle gracefully
            logger.warning(f"AI agent initialization issue: {str(e)}")
            # Continue processing - the agent will return appropriate error messages
