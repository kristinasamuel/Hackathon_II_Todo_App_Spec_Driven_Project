...
class ConversationRepository:
    ...
    @staticmethod
    def get_messages_chronological(session: Session, conversation_id: str, limit: int = 100) -> List[Message]:
        # Get messages in chronological order (oldest first)
        statement = select(Message).where(Message.conversation_id == conversation_id).order_by(Message.timestamp.asc()).limit(limit)
        return session.exec(statement).all()

    @staticmethod
    def get_latest_messages(session: Session, conversation_id: str, limit: int = 10) -> List[Message]:
        # Get the most recent messages by timestamp, then reverse for chronological order
